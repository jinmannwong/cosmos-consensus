...
func genValSetWithPowers(powers []int64) *types.ValidatorSet {
	...
	for i := 0; i < size; i++ {
		...
		val := types.NewValidator(bls12_381.GenPrivKey().PubKey(), powers[i])
...
func TestProposerPriorityDoesNotGetResetToZero(t *testing.T) {
	...
	val1PubKey := bls12_381.GenPrivKey().PubKey()
	...
	// add a validator
	val2PubKey := bls12_381.GenPrivKey().PubKey()
...
func TestLargeGenesisValidator(t *testing.T) {
	...
	genesisPubKey := bls12_381.GenPrivKey().PubKey()
	...
	firstAddedValPubKey := bls12_381.GenPrivKey().PubKey()
	...
	for i := 0; i < 10; i++ {
		addedPubKey := bls12_381.GenPrivKey().PubKey()
